...
// Hoisted to module scope so it can be exported and used in reviewController's auto-seed logic
...
/**
 * GET /api/templates
 * Returns all templates enriched with aggregated review stats.
 */
exports.getAllTemplates = async (req, res) => {
    try {
        const templates = await Template.find().sort({ createdAt: -1 }).lean();

        // Single aggregation query for all template stats — avoids N+1
        ...
        console.error('[Templates] Error fetching all templates:', error.message);
 ...
 * Returns a single template by ObjectId or short code.
 */
...
exports.useTemplate = async (req, res) => {
    try {
        ...
        console.error('[Templates] Error incrementing usage count:', error.message);
 ...
 * Idempotent seed using the code field as a unique key.
 */
exports.seedTemplates = async (req, res) => {
    try {
        ...
        for (const t of HARDCODED_TEMPLATES) {
            const exists = await Template.findOne({ code: t.code }).lean();
            if (!exists) {
        ...
        console.error('[Templates] Error seeding templates:', error.message);
        res.status(500).json({ success: false, message: 'Server error seeding templates' });
    }
};

// Exported for use in reviewController's auto-seed on demand
